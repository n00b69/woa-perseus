...
> Замените `$` номером раздела Windows (должен быть 24)
```cmd
sel par $
```
...
#### Выбhfnm раздел ESP
> Замените `$` номером раздела ESP (должен быть 23)
```cmd
sel par $
```
...
- Перетащите **magisk_patched.img** на диск **WINPERSEUS** в проводнике Windows, затем переименуйте его в **boot.img**.
